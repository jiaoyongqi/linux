...
authenticate_client(request * r)
{
    ...
    case AUTH_LLBZ:
        /* Client had X minutes to validate account by email and didn't = too late */
        debug(LOG_INFO, "Got AUTH_LLBZ from central server authenticating token %s from %s at %s "
              "- redirecting them to llbz message", client->token, client->ip, client->mac);
        safe_asprintf(&urlFragment, "%smessage=%s",
                      auth_server->authserv_msg_script_path_fragment, GATEWAY_MESSAGE_ACCOUNT_LLBZ);
        http_send_redirect_to_auth(r, urlFragment, "Redirect to llbz message");
        free(urlFragment);
        break;
