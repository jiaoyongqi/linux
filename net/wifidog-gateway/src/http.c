...
#if 0	
//closed by liubofei
        ssid = distinguish_client(mac);
        debug(LOG_INFO, "Got client MAC address for ip %s: %s", r->clientAddr, mac);
        debug(LOG_INFO,"MAC:%s from ssid:%d",mac,ssid);
        if(2 == ssid)
        {
            debug(LOG_INFO, "Got client MAC address for ip %s: %s", r->clientAddr, mac);
        ...
        {
            debug(LOG_INFO, "Got client MAC address for ip %s: %s", r->clientAddr, mac);
            //if host is using the wifi ssid2,this is used for binding driver and the device...
            safe_asprintf(&urlFragment, "%sgw_address=%s&gw_port=%d&gw_id=%s",
                          auth_server->authserv_driver_script_path_fragment,config->gw_address, config->gw_port, config->gw_id);
            free(mac);
        }
        else
        {
            debug(LOG_ERR, "Can't get client Mac!Fatal error!");
            if(!mac)free(mac);
            return;
        }
#endif
        ...
        //http_send_redirect_to_our_mainpage(r, urlFragment, "Redirect to our main page");
        free(urlFragment);
    }
    free(url);
}
...
/** @brief Convenience function to redirect the web browser to our mainpage
 * @param r The request
 * @param urlFragment The end of the auth server URL to redirect to (the part after path)
 * @param text The text to include in the redirect header ant the mnual redirect title */
void
http_send_redirect_to_our_mainpage(request * r, const char *urlFragment, const char *text)
{
    char *protocol = NULL;
    int port = 80;
    t_auth_serv *auth_server = get_auth_server();
    ...
    char *url = NULL;
    safe_asprintf(&url, "%s://%s", protocol, "192.168.0.1/home.htm");
    http_send_redirect(r, url, text);
    free(url);
}
...
void
http_callback_driver_auth(httpd * webserver, request * r)
{
    debug(LOG_INFO, "****************************************");
    debug(LOG_INFO, "driver auth succeed!");
    debug(LOG_INFO, "****************************************");
    changeSSID( 1, NULL, NULL, 2, 1);

    debug(LOG_INFO, "****************************************");
    debug(LOG_INFO, "driver auth succeed!");
    debug(LOG_INFO, "****************************************");

    http_send_redirect_to_our_mainpage(r, "设备和车辆绑定", "设备和车辆绑定成功！！");

}

void http_callback_shutdownauth(httpd *webserver,request *r)
{
    shutdownAuth();
    send_http_page(r, "关闭认证", "关闭认证成功！！！");
}
...
http_callback_auth(httpd * webserver, request * r)
{
    ...
    if ((token = httpdGetVariableByName(r, "token"))) {
        ...
        if (!(mac = arp_get(r->clientAddr))) {
            ...
            send_http_page(r, "无线验证", "抱歉！无法获取您的设备MAC地址！");
        } else {
        ...
        send_http_page(r, "验证错误", "无效的token!");
    }
}
