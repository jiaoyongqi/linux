...
#define DEFAULT_CHECKINTERVAL 30//60-->30 by liubofei
...
#define DEFAULT_AUTHSERVPATH "/chemiao/"
#define DEFAULT_AUTHSERVLOGINPATHFRAGMENT "portal/login.html?"
#define DEFAULT_AUTHSERVDRIVERPATHFRAGMENT "admin/driver.html?"
#define DEFAULT_AUTHSERVPORTALPATHFRAGMENT "portal/index.html?"
#define DEFAULT_AUTHSERVMSGPATHFRAGMENT "portal/message.html?"
#define DEFAULT_AUTHSERVPINGPATHFRAGMENT "device/ping/?"
#define DEFAULT_AUTHSERVAUTHPATHFRAGMENT "device/auth/?"
#define DEFAULT_AUTHSERVSSLCERTPATH "/etc/ssl/certs/"
//#define DEFAULT_DriverAuthScriptPathFragment "admin/driver/?"
#define DEFAULT_DRIVERAUTHSCRIPTPATHFRAGMENT "admin/driver/?"
...
#define DEFAULT_DEVICEUPLOAD 10000
#define DEFAULT_DEVICEDOWNLOAD 10000
#define DEFAULT_CLIENTUPLOAD 10000
#define DEFAULT_CLIENTDOWNLOAD 10000
...
#define FWRULESET_BLOCKED_SITES "blocked-sites"
/*@}*/
...
typedef struct _auth_serv_t {
    ...
    char *authserv_driver_script_path_fragment;  /**< @brief This is the script the driver will be sent to for login. */
...
typedef struct _mac_t {
    char *mac;
    struct _mac_t *next;
} t_trusted_mac,t_untrusted_mac,t_mac_whitelist,t_mac_blacklist,maclist;

/**
 * Popular Servers,trusted host,untrusted host
 */
typedef struct _host_t {
    char *hostname;
    struct _host_t *next;
} t_popular_server,t_host_whitelist,t_host_blacklist,hostlist;
...
typedef struct {
    ...
    int  confdirtyflag;     /**< @brief dirty flag of the config file,0 means untouched,1 means dirty */
    int  deviceauthflag;     /**< @brief device auth flag of the config file,1 means authed */
    int require_auth;       /**< @brief requireness of authentication of the device ,1 means required,0 means no */
    ...
    int deviceupload;           /**< @brief upload bandwidth of the server */

    int devicedownload;         /**< @brief download bandwidth of the server */

    int clientupload;           /**< @brief upload bandwidth of each client */

    int clientdownload;         /**< @brief download bandwidth of each client */
    ...
    t_untrusted_mac *untrustedmaclist; /**< @brief list of untrusted macs,added by vtpp */
    t_host_whitelist * hostwhitelist; /**< @brief list of allowed sites,added by vtpp*/
    t_host_blacklist * hostblacklist; /**< @brief list of blocked sites,added by vtpp*/
...
#define AUTH_SERV_FREE(s) do {\
                free((s)->authserv_hostname);\
                free((s)->authserv_path);\
                free((s)->authserv_login_script_path_fragment);\
                free((s)->authserv_driver_script_path_fragment);\
                free((s)->authserv_portal_script_path_fragment);\
                free((s)->authserv_msg_script_path_fragment);\
                free((s)->authserv_ping_script_path_fragment);\
                free((s)->authserv_auth_script_path_fragment);\
                free((s));\
} while(0)
