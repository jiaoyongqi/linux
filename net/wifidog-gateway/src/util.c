...
#define 	MAC_FILE	"/amt/WIFI/WIFI_nvram.txt"
...
char *mac_delete_colon(const char *mac)
{
	if(!mac) {
...
// 函数介绍：	获取当前设备MAC地址
// 输出参数：	mac地址
// 输入参数:	待写入mac信息的地址 mac   写入缓存空间大小mac_len
// 返回值：		0:成功	-2:参数错误	-1:获取失败
//=================================================================================

int get_device_mac(char *mac, int mac_len)
{
        FILE *fp = NULL;
        char cmd[128] = {0};
        char buf[128] = {0};
        ...
        snprintf(cmd, sizeof(cmd), "cat " MAC_FILE);
        fp = popen(cmd, "r");
        if (NULL != fp) {
                fgets(buf, sizeof(buf), fp);
                pclose(fp);
                if (sscanf(buf, "%s", mac_temp) == 0) {
                        printf("get_device_mac failed!\n");
                        return -1;
                }
                strncpy(mac, mac_temp, mac_len < sizeof(mac_temp)? mac_len : sizeof(mac_temp));
                return 0;
        } else {
                printf("cat mac file error!\n");
                return -1;
        }
}
