...
iptables_load_ruleset(const char *table, const char *ruleset, const char *chain)
{
    ...
    for (rule = get_ruleset(ruleset); rule != NULL; rule = rule->next) {
        ...
        debug(LOG_DEBUG, "before free cmd");
        
        free(cmd);
        debug(LOG_DEBUG, "after free cmd");
    }
...
//this function to clear whitelist is added by vtpp,for hosts has previledge ...
void
iptables_fw_clear_hostwhitelist(void)
{
    iptables_do_command("-t filter -F " CHAIN_HOSTWHITELIST);
    iptables_do_command("-t nat -F " CHAIN_HOSTWHITELIST);
}

void iptables_fw_handle_black_host(int flag,char *host)
{
    //flag = 1 means add ,0 means delete...
    char cmd[256] ={0};
    sprintf(cmd,"%s FORWARD -d %s -j REJECT",flag ? "-I":"-D",host);
    iptables_do_command(cmd);
}

void iptables_fw_flush_black_host()
{
    char cmd[256] ={0};
    sprintf(cmd,"-F FORWARD ");
    iptables_do_command(cmd);
}
...
//this function is added by vtpp to add rules for host in whitelist...
void
iptables_fw_set_hostwhitelist(void)
{
    const s_config *config;
    t_host_whitelist *whitelist;

    config = config_get_config();

    for (whitelist = config->hostwhitelist; whitelist != NULL; whitelist = whitelist->next) {
        if (whitelist->hostname && strcmp(whitelist->hostname, "0.0.0.0") != 0) {
            iptables_do_command("-t filter -A " CHAIN_HOSTWHITELIST " -d %s -j ACCEPT", whitelist->hostname);
            iptables_do_command("-t nat -A " CHAIN_HOSTWHITELIST " -d %s -j ACCEPT", whitelist->hostname);
        }
    }
}

static unsigned int ip_start = 100;
static unsigned int ip_end = 105;
...
iptables_fw_init(void)
{
    ...
    for (p = config->untrustedmaclist; p != NULL; p = p->next)
        iptables_do_command("-t mangle -A " CHAIN_TRUSTED " -m mac --mac-source %s -j MARK --set-mark %d", p->mac,
                            FW_MARK_KNOWN);
    
    for( ip_counter = ip_start;ip_counter < ip_end;ip_counter++)
    {
        iptables_do_command("-t mangle -A POSTROUTING -o %s -s 192.168.0.%d -j MARK --set-mark 3%d",config->external_interface,ip_counter,ip_counter);
    }
    ...
    iptables_do_command("-t nat -N " CHAIN_HOSTWHITELIST);
    if (got_authdown_ruleset)
    ...
    //added by vtpp...
    iptables_do_command("-t nat -A " CHAIN_UNKNOWN " -j " CHAIN_HOSTWHITELIST);
    if (got_authdown_ruleset) {
    ...
    iptables_do_command("-t filter -N " CHAIN_HOSTWHITELIST);
    ...
    iptables_do_command("-t filter -A " CHAIN_TO_INTERNET " -j " CHAIN_HOSTWHITELIST);
    iptables_fw_set_hostwhitelist();
...
static void  iptables_fw_clear_unknowns()
{
...
int iptables_fw_allowunknown()
{
    iptables_fw_clear_unknowns();
    iptables_fw_clear_authservers();
    return 0;
}

void fw_clear_tc_setups()
{
    const s_config *config;
    LOCK_CONFIG();
    config = config_get_config();
    ...
    { 
        iptables_do_command("-t mangle -D POSTROUTING -o %s -s  192.168.0.%d -j MARK --set-mark 3%d",config->external_interface,p,p);
    }
    UNLOCK_CONFIG();
}
...
iptables_fw_destroy(void)
{
    ...
    //added by vtpp...
    iptables_do_command("-t nat -X " CHAIN_HOSTWHITELIST);
    ...
    //added by vtpp ...
    iptables_do_command("-t filter -X " CHAIN_HOSTWHITELIST);
...
int
iptables_fw_access_host_vtpp(fw_access_t type, const char *host)
{
    int rc;
...
int iptables_demonish_chain(const char * chain)
{
    char *command = NULL;
    safe_asprintf(&command, "-t nat -F %s", chain);
    iptables_insert_gateway_id(&command);
    debug(LOG_INFO,command);
    iptables_do_command(command);
    safe_asprintf(&command, "-t filter -F %s", chain);
    iptables_insert_gateway_id(&command);
    iptables_do_command(command);
    return 0;
}

int iptables_create_chain(char *ruleset,char *chain)
{
    char *rs = NULL,*cn = NULL; 

    safe_asprintf(&rs, "%s", ruleset);
    iptables_insert_gateway_id(&rs);
    debug(LOG_INFO,"ruleset:%s",rs);

    safe_asprintf(&cn, "%s", chain);
    iptables_insert_gateway_id(&cn);
    debug(LOG_INFO,"chain:%s",cn);

    iptables_load_ruleset("filter", rs, cn);

    debug(LOG_INFO,"before nat");
    iptables_load_ruleset("nat", rs, cn);
...
int iptables_shutdown_auth()
{
	iptables_demonish_chain(CHAIN_GLOBAL);
    iptables_fw_allowunknown();
	return 0;
}
