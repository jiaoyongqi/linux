...
main_loop(void)
{
    ...
    if (!config->gw_id) {
        ...
        if (0 == get_device_mac(mac_tmp,32))
        {
            config->gw_id = mac_delete_colon(mac_tmp);
        }
        else
        {
    ...
    httpdAddCContent(webserver, "/wifidog", "driverauth", 0, NULL, http_callback_driver_auth);
    httpdAddCContent(webserver, "/wifidog", "disconnect", 0, NULL, http_callback_disconnect);
    httpdAddCContent(webserver, "/wifidog", "testshutdownauth", 0, NULL, http_callback_shutdownauth);
    ...
    while (1) {
        ...
        if (webserver->lastError == -1) {
            ...
            *(params + 1) = r;
             ...
             * we don't set any... */
            debug(LOG_ERR, "FATAL: We failed an ACL.... No handling because we don't set any...");
        }
    }
