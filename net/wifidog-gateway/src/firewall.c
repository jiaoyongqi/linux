...
/**
 * Deny a host through the firewall by removing a rule in the firewall
 * @param host IP address, domain or hostname to deny
 * @return Return code of the command
 */
int
fw_deny_host(const char *host)
{
    debug(LOG_DEBUG, "Denying %s", host);

    return iptables_fw_access_host(FW_ACCESS_DENY, host);
}
...
fw_init(void)
{
    ...
    debug(LOG_INFO,"test host blacklist");
    //iptables_fw_handle_black_host(1,"m.baidu.com");
    return result;
}
 ...
 */
void
fw_clear_hostwhitelist(void)
{
    debug(LOG_INFO, "Clearing the host whitelist");
    iptables_fw_clear_hostwhitelist();
}

/** Add the necessary firewall rules to whitelist the authservers
 */
void
fw_set_hostwhitelist(void)
{
    debug(LOG_INFO, "Setting the host whitelist");
...
fw_sync_with_authserver(void)
{
                    ...
                    if (tmp->fw_connection_state != FW_MARK_KNOWN) {
                        ...
                        if (tmp->fw_connection_state != FW_MARK_PROBATION) {
                            tmp->counters.incoming_delta =
                             ...
                             //tmp->onlinetime = tmp->downloadspeed = tmp->uploadspeed = 0;
                        } else {
	...
	case AUTH_LLBZ:
			 debug(LOG_NOTICE, "AUTH_LLBZ--code=7-->%s - Denied. Removing client and firewall rules", tmp->ip);
			 fw_deny(tmp);
			 client_list_delete(tmp);
			 break;
